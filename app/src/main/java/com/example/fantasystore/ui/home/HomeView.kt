...
import com.example.fantasystore.ui.common.ScreenContent
import com.example.fantasystore.ui.theme.FantasyStoreTheme
...
fun ConversionForm(
    ...
    val input1Amount by viewModel.input1.amount.collectAsState() // Observa el valor del input 1
    val input2Amount by viewModel.input2.amount.collectAsState() // Observa el valor del input 2

    Log.d("ConversionForm", "Result: $result")
    Log.d("ConversionForm", "Input1 Amount: $input1Amount")
    Log.d("ConversionForm", "Input2 Amount: $input2Amount")
    ...
    Column(horizontalAlignment = Alignment.CenterHorizontally) {
        CurrencyRow(
            ...
            onAmountChange = { newAmount ->
                ...
                if (newAmount.isEmpty()) viewModel.onAmountChange(SECOND_QTY_INPUT, EMPTY_STRING) // Clear the other input
            }, // Pass index 0
            amount = if (viewModel.updateSource == HomeViewModel.UpdateSource.INPUT2 && result != null) {
                Log.d("ConversionForm", "Setting amount for CurrencyRow 1 to ${result.toString()}")
                result.toString()
            } else {
                Log.d("ConversionForm", "Setting amount for CurrencyRow 1 to $input1Amount")
                input1Amount
            }
        )
        CurrencyRow(
            ...
            onAmountChange = { newAmount ->
                ...
                if (newAmount.isEmpty()) viewModel.onAmountChange(FIRST_QTY_INPUT, EMPTY_STRING) // Clear the other input
            }, // Pass index 1
            amount = if (viewModel.updateSource == HomeViewModel.UpdateSource.INPUT1 && result != null) {
                Log.d("ConversionForm", "Setting amount for CurrencyRow 2 to ${result.toString()}")
                result.toString()
            } else {
                Log.d("ConversionForm", "Setting amount for CurrencyRow 2 to $input2Amount")
                input2Amount
            }
        )
...
fun CurrencyRow(
    ...
    Log.d("CurrencyRow", "index: $index Amount: $amount,textFieldValue: $textFieldValue")
        ...
        Row(
            ...
            TextField(
                ...
                onValueChange = { newText ->
                    ...
                    textFieldValue = filteredText // Update textFieldValue
                    onAmountChange(filteredText) // Trigger onAmountChange
                },
